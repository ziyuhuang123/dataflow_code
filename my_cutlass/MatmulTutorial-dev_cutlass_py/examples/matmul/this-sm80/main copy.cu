...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < M; ++i)
    {
        for (int j = 0; j < K; ++j)
        {
            hA[i * K + j] = (half)((rand() % 1000 * 1 / 100 % 10 + 0.0)*1e-1);
    ...
    for (int k = 0; k < K; ++k)
    {
        for (int n = 0; n < N; ++n)
        {
            hB[n * K + k] = (half)((rand() % 1000 * 1 / 100 % 10 + 0.0)*1e-1);
    ...
    for (int n = 0; n < N; ++n)
    {
        for (int t = 0; t < T; ++t)
        {
            h_gemm1_weight[t * N + n] = (half)((rand() % 1000 * 1 / 100 % 10 + 0.0)*1e-1);
            // h_gemm1_weight[t * N + n] = (half)(1e-1);
            // if(n<128){
            //     h_gemm1_weight[t * N + n] = (half)(2*1e-1);
            // }
            // else{
                // h_gemm1_weight[t * N + n] = (half)(1*1e-1);
            // }
            // h_gemm1_weight[t * N + n] = (half)((t * N + n)*1e-3);
...
//     std::cout << "Golden111 values done!\n";
    ...
    half *dB_cublas;
    half *dC_cublas;
    half *d_gemm1_weight_cublas;
    half *d_gemm1_result_cublas;

    CUDA_CHECK(cudaMalloc(&dA_cublas, M * K * 2));
    CUDA_CHECK(cudaMalloc(&dB_cublas, K * N * 2));
    CUDA_CHECK(cudaMalloc(&dC_cublas, M * N * 2));
    CUDA_CHECK(cudaMalloc(&d_gemm1_weight_cublas, N * T * 2));
    CUDA_CHECK(cudaMalloc(&d_gemm1_result_cublas, M * T * 2));

    CUDA_CHECK(cudaMemcpy(dA_cublas, hA, M * K * 2, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(dB_cublas, hB, K * N * 2, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(dC_cublas, hC, M * N * 2, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_gemm1_weight_cublas, h_gemm1_weight, T * N * 2, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_gemm1_result_cublas, h_gemm1_result, M * T * 2, cudaMemcpyHostToDevice));
    ...
    gemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, M, N, K, &alpha, dA_cublas, K, dB_cublas, K, &beta, dC_cublas, M);

    gemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, M, T, N, &alpha, dC_cublas, M, d_gemm1_weight_cublas, N, &beta, d_gemm1_result_cublas, M);

    CUDA_CHECK(cudaMemcpy(golden1, d_gemm1_result_cublas, M * T * 2, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(golden, dC_cublas, M * N * 2, cudaMemcpyDeviceToHost));
    ...
    dim3 dimGrid(cluster_size, M / 128); // 增加这个是因为T维度上只需要一个block -- 这里的2是cluster.x的尺寸。不过也要求N大于2个block
...
//   CUDA_CHECK(launch_result);
  ...
  CUDA_CHECK(launch_result);
    ...
    int smem_size = MAX(STAGES * 128 * 32 * 2 * 2, 128 * 128 * 2+128 * 128 * 2+128*32*2);// 第一个128*128*2是GEMM0的中间结果。第二个128*128*2是GEMM1的结果。之前会预取。现在就取消预取。直接多个block累加。然后各自存储出去。
    std::cout << "Using shared memory = " << (double)smem_size / 1e3 << " KB.\n";
    if (smem_size >= (48 << 10))
    {
        CUDA_CHECK(cudaFuncSetAttribute(matmul,
    ...
    dim3 cluster(cluster_size, 1, 1);
        void* kernel_params[] = {
            (void*)&dA, (void*)&dB,(void*)&dC, (void*)&d_gemm1_result, (void*)&d_gemm1_weight,
            (void*)&M, (void*)&N, (void*)&K, (void*)&T,
            (void*)&alpha, (void*)&beta
        };
    cudaLaunchConfig_t launch_config;
    ...
    float ms;
    cudaEventElapsedTime(&ms, start, stop);
    std::cout << "Running cost (ms) of matmul is " << ms/iterations << "(ms)\n";

    // 计算TFLOPS

    float tflops = ((float)2 * M * N * K + (float)2 * M * T * N) / (ms / iterations) * 1e3 / 1e12;
    printf("TFLOPS=%f\n", tflops);
    ...
    for (int i = 0; i < M; ++i)
    {
        for (int j = 0; j < N; ++j)
        {
            ...
            float diff = ((float)golden[j * M + i] - (float)hC[i * N + j]);
            if (diff < 0)
            {
                diff = -diff;
            }
            // float maxv = MAX((float)golden[i * N + j], (float)hC[i * N + j]);
            float maxv = MAX((float)golden[j * M + i], (float)hC[i * N + j]);
            if (maxv < 0)
            {
                maxv = -maxv;
            }
    ...
    for (int i = 0; i < M; ++i)
    {
        for (int j = 0; j < T; ++j)
        {
            ...
            float diff = ((float)golden1[j * M + i] - (float)h_gemm1_result[i * T + j]);
            if (diff < 0)
            {
                diff = -diff;
            }
            // float maxv = MAX((float)golden1[i * T + j], (float)h_gemm1_result[i * T + j]);
            float maxv = MAX((float)golden1[j * M + i], (float)h_gemm1_result[i * T + j]);
            if (maxv < 0)
            {
                maxv = -maxv;
            }
