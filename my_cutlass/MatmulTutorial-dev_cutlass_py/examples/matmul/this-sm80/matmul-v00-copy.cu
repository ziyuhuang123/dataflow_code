...
__device__ void loadSmemC(half *smem, half *C, int M, int N, int N_index)
{
    // load 128 * 128
    int bx = blockIdx.x;
    bx = N_index;
...
__device__ void loadFragA(nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, wmmaM, wmmaN, wmmaK, half, nvcuda::wmma::row_major> *frag, half *smem, int ki)
{
    ...
    } // 有点奇怪。。我们有4个warp。load_matrix_sync就需要分给四个不同的warp去工作吧。这里。。只分配了threadIdx.z?就是2个？
}
...
{ // 中间结果彻底不需要存出去了。每次都完全利用干净。gemm1_weight的尺寸是N*T（col-major)
    ...
    half *S_gemm1_result = reinterpret_cast<half *>(shared_storage+ (MI * NI+MI*KI) * sizeof(half)); // 这里不能复用哈。GEMM0的值算出来得留着，然后又需要去累加GEMM1的值。除非是计算完之后再去加GEMM1的上一次的值。那样可以刷掉GEMM0结果的空间
    ...
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, wmmaM, wmmaN, wmmaK, half> Accum[MII / wmmaM * NII / wmmaN]; //4*4
...
// 自从N_iter的引入，每个block就负责一整行了。也就是loadSmemB要修改了。
        ...
        if(N_iter>=1){
            loadSmemC(S_gemm1_result, gemm1_result, M, T, N_iter);
