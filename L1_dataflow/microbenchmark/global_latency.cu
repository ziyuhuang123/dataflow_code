...
#define ARRAY_SIZE 10485760 //(10485760 4090) (52428800 A100) (62914560 H800)  //pointer-chasing array size in 64-bit. total array size is 7 MB which larger than L2 cache size (6 MB in Volta) to avoid l2 cache resident from the copy engine
#define BLOCKS 128			//(128 4090) (108 A100) (114 H800)
...
__global__ void mem_lat(uint32_t *startClk, uint32_t *stopClk, uint64_t *posArray, uint64_t *dsink)
{
	...
	{ // only THREADS_NUM has to be active here
		...
		// initialize the pointers with the start address (maybe can also be seem as warm-up)
		// Here, we use cache volatile modifier to ignore the L2 cache
		asm volatile("{\t\n"
					 "ld.global.cv.u64 %0, [%1];\n\t"
					 "}" : "=l"(ptr1) : "l"(ptr) : "memory");

// Why we add "::memory" within PTX line: Normally any memory that is written to will be specified as an out operand, but if there is a hidden side effect on user memory (for example, indirect access of a memory location via an operand), or if you want to stop any memory optimizations around the asm() statement performed during generation of PTX, you can add a “memory” clobbers specification after a 3rd colon, e.g.:
