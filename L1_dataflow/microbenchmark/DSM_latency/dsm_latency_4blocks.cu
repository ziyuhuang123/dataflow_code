...
__global__ void dsm_sm2sm_lat_kernel(uint *d_Data, uint array_size)
{
    ...
    cluster.sync();
    if ((threadIdx.x == 0))
        printf("1: smid:%u, rank:%u: initial-value: %u\n", smid, clusterBlockRank, smem[0]);
    ...
    if (threadIdx.x == 0 && clusterBlockRank==0)
        printf("2: smid:%u, rank:%u: check-value: %u\n", smid, clusterBlockRank, smem[0]);

    atomicAdd(d_Data, smem[0]);

    if (threadIdx.x == 0 && clusterBlockRank==0)
        printf("3: smid:%u, rank:%u: stop-start=%ld-%ld: SM-to-SM latency = %ld\n", smid, clusterBlockRank, start, end, (end - start) / array_size);
}
...
extern "C" void dsm_sm2sm_latency(void *d_Data, uint arraySize)
{
    ...
    int cluster_size = 2;  // cluster size并不是包含多少个grid，而还是包含多少个block。所以gridDim要大于等于cluster_size
