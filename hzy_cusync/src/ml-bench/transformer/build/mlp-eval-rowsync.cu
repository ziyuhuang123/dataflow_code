...
#else
//<eval tiles>
using ShapeThreadBlock1 = cutlass::gemm::GemmShape<128, 256, 64>;  
using ShapeWarp1 = cutlass::gemm::GemmShape<64, 64, 64>;
using ShapeThreadBlock2 = cutlass::gemm::GemmShape<128, 256, 64>;  
using ShapeWarp2 = cutlass::gemm::GemmShape<64, 64, 64>;
...
struct MLPParameters {
  ...
  MLPParameters(std::string model_, uint batch, bool check) {
    ...
    if (model == "gpt3") {
      // gemm_size1 = cutlass::gemm::GemmCoord(batch, 512, 128);
      // gemm_size2 = cutlass::gemm::GemmCoord(batch, 512, 128);
...
cudaError_t checkMLPResults(MLPParameters& mlpParams) {
  ...
  bool eq = equals(mlpParams.ref_xw1.size(), mlpParams.ref_xw1.host_data(), hostC, 0.01);
  if (eq == false) {
  ...
  eq = equals(mlpParams.ref_xw12.size(), mlpParams.ref_xw12.host_data(), hostE, 0.01);
...
// 安全性：使用 constexpr 可以确保这些常量在整个程序生命周期中都是不可变的，提高了代码的安全性和可维护性。--->没有这个会报错说这个不是常量。。。原先是在device/cusyncgemm.h里面的template。template是在编译时就自动确定下来的。这里要是直接写int AlignmentA，就是在运行时确定。不过加上constexpr就OK啦
  ...
  static int const k_AlignmentA = AlignmentA;
  static int const k_AlignmentB = AlignmentB;
  ...
  using GemmKernel = typename cutlass::gemm::kernel::DefaultCuSyncGemm<ElementInputA, LayoutInputA, k_AlignmentA, ElementInputB, LayoutInputB, k_AlignmentB, ElementOutput, LayoutOutput, ElementAccumulator, MMAOp, SmArch, ShapeThreadBlock1, ShapeWarp1, ShapeMMAOp, EpilogueOp1, CuSyncGeMMSwizzle, NumStages1, false, Operator_, SharedClearOption, false, false, false, PermuteDLayout >::GemmKernel;
  ...
  typename GemmKernel::Params<ProdCuStage> prod_params{prod, args1.problem_size, grid_shape1, args1.ref_A.non_const_ref(), args1.ref_B.non_const_ref(), args1.ref_C.non_const_ref(), args1.ref_D, args1.epilogue, reinterpret_cast<int *>(workspace1.get()), args1.gather_A_indices, args1.gather_B_indices, args1.scatter_D_indices};

  typename GemmKernel::Params<ConsCuStage> cons_params{cons, args2.problem_size, grid_shape2, args2.ref_A.non_const_ref(), args2.ref_B.non_const_ref(), args2.ref_C.non_const_ref(), args2.ref_D, {mlpParams.alpha, mlpParams.beta}, reinterpret_cast<int *>(workspace2.get()), args2.gather_A_indices, args2.gather_B_indices, args2.scatter_D_indices};

  dim3 grid = {grid_shape1.n()*grid_shape1.m()+grid_shape2.n()*grid_shape2.m(), 1, 1};
