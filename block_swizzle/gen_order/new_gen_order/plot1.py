...
def generate_zigzag_order_0(M, N, T1):
    ...
    for t in t_combo:
        for j in tqdm(range(M), desc=f"Processing rows for t={t}"):
            for k in range(t):
...
with open('orders.txt', 'w') as f:
    ...
    for i in tqdm(range(1, 9), desc="Processing outer_width and inner_width"):
        ...
        f.write(f'outer_width = {outer_width}, inner_width = {inner_width}\n')
        
        all_orders_4 = generate_zigzag_order_4(M, N, outer_width, inner_width)
        f.write('Order 4:\n')
        for order in all_orders_4:
            f.write(f'{order}\n')
