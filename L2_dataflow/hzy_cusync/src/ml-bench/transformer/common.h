...
bool equals(size_t size, T* mat1, T* mat2, float err) {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    // 计算相对误差
    float relative_err = abs(e1 - e2) / max(abs(e1), abs(e2));
    bool ret = (relative_err <= err);

    if(i==0){
      printf("%f != %f at %lu, relative_err=%f\n", e1, e2, i, relative_err);
    }
...
//     } else if (abs(e1) < v) {
//       ret = false;
//       printf("117: %f , %f at %lu\n", e1, e2, i);
//     } else if (abs(e2) < v) {
//       ret = false;
//       printf("120: %f , %f at %lu\n", e1, e2, i);
//     } else {
//       float err = abs(abs(e1) - abs(e2))/max(abs(e1), abs(e2));
//       if (err <= v) {
//         ret = true;
//       } else {
//         printf("126: %f , %f at %lu, %f\n", e1, e2, i, err);
//         ret = false;
//       }
//     }
